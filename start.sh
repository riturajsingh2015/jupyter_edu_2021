...
if [ $(id -u) == 0 ] ; then
    # Only attempt to change the student username if it exists
    if id student &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER student
    fi
    ...
    if [[ "$NB_USER" != "student" ]]; then
        ...
        if [[ ! -e "/home/$NB_USER" ]]; then
            ...
            mv /home/student "/home/$NB_USER"
        fi
        # if workdir is in /home/student, cd to /home/$NB_USER
    ...
    if [[ "$NB_UID" == "$(id -u student)" && "$NB_GID" == "$(id -g student)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^student:/nayvoj:/" > /tmp/passwd
